...
class dataLoader():
    def __init__(self, config):
        ...
        if config.isTrain:
            ...
            self.uhisMatTra, self.tarMatTra = self.generateHis(self.trainList, isTrain=1, isEval=0)
            self.uhisMatVal, self.tarMatVal = self.generateHis(self.validList, isTrain=1, isEval=1)
            print('Mat done')

        else:
            # generate and store the matrices in the first run and just load these matrices in the following runs
            self.uhisMatTraVal, self.tarMatTraVal = self.generateHis(self.trainValList, isTrain=0, isEval=0)
            self.uhisMatTest, self.tarMatTest = self.generateHis(self.testList, isTrain=0, isEval=1)
            print('Mat done')
